...
def data_init(config: SolidConfig, size: int, rngs: PRNGKey) -> BatchedData[SolidData]:
    ...
    prim_atom_coords = jnp.stack([atom.coords_array for atom in atoms])
    prim_atom_charges = jnp.array([atom.charge for atom in atoms])

    S = jnp.array(config.supercell_matrix)
    Rpts = get_supercell_copies(jnp.array(config.lattice_vectors), S)
    super_atom_coords = Rpts[:, None, :] + prim_atom_coords[None, :, :]
    super_atom_coords = super_atom_coords.reshape(-1, 3)
    super_atom_charges = jnp.tile(prim_atom_charges, len(Rpts))

    rngs_position, rngs_spins = jax.random.split(rngs)
    initial_spins_per_atom = distribute_spins(
        rngs_spins, atoms, config.per_atom_init, config.electron_spins
    )
    electron_positions = initialize_electrons_gaussian(
        rngs_position,
        super_atom_coords,
        initial_spins_per_atom * config.scale,
        size,
