...
In `forward_laplacian` mode, the Hessian is not available, so the angular momentum observables are evaluated in a separate pass. For each of the three Cartesian rotation axes, nested JVPs compute the first and second directional derivatives of $\log\psi$ along the corresponding global rotation field. This directly supplies the derivatives needed by $\hat L_a^2$ without constructing the full coordinate Jacobians produced by a reverse-mode implementation of $\hat{\mathbf L}$, reducing the derivative work and intermediate storage.
