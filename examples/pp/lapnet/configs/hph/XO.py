"""
Input configuration generator for a dimer transition-metal atom with non metal single atom (X-O)
using the hybrid pseudohamiltonian (HPH) potential in JaQMC.

This input:
1. Reads pseudopotential data of local and L2 part from XML PH files and 
   residual nonlocal part from NWChem-format ECP file.
...
Returns:
    cfg: JaQMC ConfigDict describing the a dimer transition-metal atom with non metal single atom (X-O) system with HPH pseudopotential.
"""
...
pseudopotential files (XML and NWChem-format ECP files).
"""
...
def get_config(input_str: str):
    ...
    symbol, spin, pp_name, pp_type = (parts[0], int(parts[1]), parts[2], parts[3])

    # make ecp data
    # based on the Sc.0.21_187_nl.nwchem
    # must have nonlocal part only for ECP
    ecp_path = f"{PP_DIRECTORY}/{symbol}.{pp_name}_nl.nwchem"
    ...
    mol = gto.Mole()
    mol.build(
        atom=f"{symbol} 0 0 0 ; O 0 0 1.664", # X-O distances
    ...
    # add ph_type to config
    cfg.ecp.ph_element = (symbol,)
