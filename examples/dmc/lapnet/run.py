...
'''
A show case for LapNet & LapJax (Forward-Laplacian) integration.
...
def run_wrapper(cfg, dmc_cfg):
    atoms, charges, nspins = get_molecule_configuration(cfg)
    key = jax.random.PRNGKey(666 if cfg.debug.deterministic else int(1e6 * time.time()))

    _, network, *_ = networks.network_provider(cfg)(atoms, nspins, charges)
    ...
    local_energy_func = lambda x: local_energy(params=single_params, key=None, data=x)
    ...
    run(
        ...
        local_energy_func=local_energy_func,
    )
